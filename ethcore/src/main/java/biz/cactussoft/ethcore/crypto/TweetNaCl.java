package biz.cactussoft.ethcore.crypto;
...
public class TweetNaCl {
    ...
    public static byte[] secretbox(byte[] message, byte[] nonce, byte[] key) {
        byte[] m = new byte[SECRETBOX_INTERNAL_OVERHEAD_BYTES + message.length];
        byte[] c = new byte[m.length];
        System.arraycopy(message, 0, m, SECRETBOX_INTERNAL_OVERHEAD_BYTES, message.length);
